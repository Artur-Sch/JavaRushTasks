	...
	public static void main(String[] args)throws ParseException{
		if(i>0){
			if("c".equals(args[0])){
				...
				System.out.println("["+i+"] >>>>>>"+Arrays.toString(args)+"<<<<<<");
				if("м".equals(args[2])) allPeople.add(Person.createMale(args[1],new SimpleDateFormat("dd/MM/yyyy").parse(args[3])));
				else if("ж".equals(args[2])) allPeople.add(Person.createFemale(args[1],new SimpleDateFormat("dd/MM/yyyy").parse(args[3])));
				System.out.println(allPeople.size()-1);
		...
		if(++i==1){
			String[] a={"c","Имя1","м","01/08/2035"};
			main(a);
		}else if(i==2){
			String[] a={"u","1","Имя2","ж","04/06/2004"};
			main(a);
		}else if(i==3){
			String[] a={"d","1"};
			main(a);
